...
# Document pre-processing for BM25
def preprocess_documents(documents: List[str]) -> List[List[str]]:
    return [re.findall(r'\b[\w\'-]+\b', doc.lower()) for doc in documents]
...
# Retrieve top-k relevant documents for a query using the BM25 algorithm
def retrieve_documents_bm25(query: str, documents: List[str], k: int = 5) -> List[Dict]:
    tokenized_docs = preprocess_documents(documents)
    bm25 = BM25Okapi(tokenized_docs)
    tokenized_query = re.findall(r'\b[\w\'-]+\b', query.lower())
    bm25_scores = bm25.get_scores(tokenized_query)
    top_k_indices = np.argsort(bm25_scores)[::-1][:k]
    return [{"doc": documents[idx], "score": bm25_scores[idx]} for idx in top_k_indices]
...
def run_base_rag_experiment(questions: List[Dict], documents: List[str], doc_embeddings: np.ndarray) -> List[Dict]:
    ...
    for q in questions:
        ...
        retrieved_docs = retrieve_documents_bm25(query, documents, k=5) 
